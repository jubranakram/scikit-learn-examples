# -*- coding: utf-8 -*-
"""
about: elastic net regression (train-test size)
...
if __name__ == '__main__':
    ...
    for idx in range(num_tests):
        ...
        for idx_1 in range(num_alpha):
            e_reg = ElasticNet(alpha=alpha[idx_1], max_iter=5000)
            ...
            score_train[idx, idx_1] = e_reg.score(X_train, y_train)
            score_test[idx, idx_1] = e_reg.score(X_test, y_test)
       
    marker_detail = ['-ok', '-vk', '-^k', '-pk']   
    for idx in range(num_alpha):
        plt.plot(test_size, score_train[:, idx], marker_detail[idx], markerfacecolor='red', label=r'train ($\alpha$ = ' + '{})'.format(alpha[idx]))
        plt.plot(test_size, score_test[:, idx], marker_detail[idx], markerfacecolor='blue', label='test')
